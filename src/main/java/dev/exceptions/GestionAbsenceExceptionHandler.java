...
public class GestionAbsenceExceptionHandler {
	...
	protected ResponseEntity<Object> handleConflictModificationInvalideException(RuntimeException ex,
			WebRequest request) {
		String bodyOfResponse = "Modification invalide: " + ModificationInvalideException.message;
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(bodyOfResponse);
	}

	@ExceptionHandler(value = { DepartementInvalideException.class })
	protected ResponseEntity<Object> handleConflictDepartementInvalideException(RuntimeException ex,
			WebRequest request) {
		String bodyOfResponse = "Departement invalide: " + DepartementInvalideException.message;
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(bodyOfResponse);
	}
}
